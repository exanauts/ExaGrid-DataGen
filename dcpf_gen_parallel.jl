...
function parse_cli()
    ...
    opts[:output_dir] = parsed["output_dir"] == "" ? joinpath("results_dc_pf", parsed["instance"]) : parsed["output_dir"]
...
@everywhere begin
    ...
    function solve_dcpf_scenario(base_network, scenario_id, p_range, q_range, solver_sym, seed_offset)
        network = deepcopy(base_network)
        perturb_loads_separate!(network, p_range, q_range, seed_offset + scenario_id)

        pm = instantiate_model(network, DCPPowerModel, PowerModels.build_pf)

        if solver_sym == :ipopt
            JuMP.set_optimizer(pm.model, Ipopt.Optimizer)
        else
            error("Unsupported solver for DC power flow on worker: $(solver_sym)")
        end

        result = try
            optimize_model!(pm)
        catch err
            println("DC power flow scenario $(scenario_id) failed with solver $(solver_sym):")
...
println("Solving $(opts[:n_scenarios]) DC power flow scenarios on $(nworkers()) workers...")
...
for chunk_idx in 1:n_chunks
    ...
    chunk_results = pmap(i -> solve_dcpf_scenario(base_network, i, opts[:p_range], opts[:q_range], opts[:solver], opts[:seed_offset]), scenario_range)
...
println("DC power flow scenario generation complete. Outputs stored in $(opts[:output_dir])")
