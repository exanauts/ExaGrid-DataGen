...
if opts[:solver] == :ipopt
    ...
    error("Unsupported solver for DC power flow: $(opts[:solver])")
end
...
try
    ...
    println("dcpf $(opts[:solver]) [$(result["termination_status"])]: $(opts[:instance])")
    ...
    println("Error during DC power flow of $(opts[:instance]) with solver $(opts[:solver])")
