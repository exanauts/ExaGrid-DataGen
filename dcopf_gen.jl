...
if opts[:solver] == :ipopt
    ...
    error("Unsupported solver for DC OPF: $(opts[:solver])")
end
...
try
    ...
    println("dcopf $(opts[:solver]) [$(result["termination_status"])]: $(opts[:instance]) objective=$(result["objective"])")

    if opts[:write_output]
        total_power_slack = 0.0
        total_line_slack = 0.0
        write_scenario_to_hdf5(opts[:output_file], network, result, 1, total_power_slack, total_line_slack)
        verify_hdf5_structure(opts[:output_file])
    end
catch err
    println("Error during DC OPF optimization of $(opts[:instance]) with solver $(opts[:solver])")
